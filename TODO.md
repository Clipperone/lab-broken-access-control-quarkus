...
reintrodotte con marcatori `// VULNERABILITY: (n)`. Eseguendo `mvn verify -P security` sul branch
`branch-vulnerable` i 26 test negativi di sicurezza vanno in rosso; le soluzioni si trovano sul branch
sanato (`main` / `feature/security-testing-training`).
