...
- **Appuntamenti**: gestione spostata da `/doc/appointment` a **`/scientist/appointment`** (breaking change sugli endpoint). Gli appuntamenti vivono ora nel dominio `/scientist`, coerente con il registro scienziato→ufficio da cui è derivato l'ufficio dell'appuntamento. Cambia solo il path REST: comportamento di sicurezza, entità/tabella `APPOINTMENT`, repository, DTO e nomi classe (`AppointmentResource`/`AppointmentResourceTest`) restano invariati. Aggiornati test, GUI e documentazione.
